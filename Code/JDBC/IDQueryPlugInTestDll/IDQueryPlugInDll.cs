...
namespace IDQueryPlugInTestDll
{
    /// <summary>
    /// 根据ID查询entity
    /// </summary>
    public class IDQueryPlugInDll : IQueryPlugIn
    {
        public string Name
        {
        ...
        public IDQueryPlugInDll(CoreService coreService)
        {
        ...
        public async Task<IEnumerable<JDBCEntity>> FindJdbcEntityAsync(string query)
        {
            ...
            if (index > 0) //存在?(子节点查询)
            {
                if (Guid.TryParse(query.Substring(4, index - 4), out parentId)) // 获取{id}
                {
            ...
            else if (query.LastOrDefault().Equals('/') && query.Length == 4) //返回空列表
            {
                return result;
            }
            else // 不存在?，仅根据{id}查询节点
            {
                if (Guid.TryParse(query.Substring(4), out parentId)) // 获取{id}
                {
                    parent = await myCoreService.GetOneByIdAsync(parentId);
                    if (parent == null)
                    {
